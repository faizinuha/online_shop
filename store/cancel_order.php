...
if (isset($_GET['id'])) {
    ...
    $update = mysqli_query($koneksi, "UPDATE orders SET status = 'cancelled' WHERE id = '$order_id'");

    if ($update) {
        echo '<script>alert("Order cancelled successfully!"); window.location.href="orders.php";</script>';
    } else {
        echo '<script>alert("Failed to cancel order. Please try again."); window.location.href="orders.php";</script>';
    }
} else {
    echo '<script>alert("Invalid order."); window.location.href="orders.php";</script>';
}
